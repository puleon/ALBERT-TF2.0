...
def create_training_instances(input_files, tokenizer, max_seq_length,
                              ...
                              max_predictions_per_seq, rng, mem_size):
    ...
    for document_index in trange(len(all_documents)):
      instances.extend(
          create_instances_from_document(
              ...
              masked_lm_prob, max_predictions_per_seq, vocab_words, rng, mem_size))
...
def create_instances_from_document(
  ...
  max_num_tokens = max_seq_length - 3 - mem_size
...
def create_masked_lm_predictions(tokens, masked_lm_prob,
  ...
  for (i, token) in enumerate(tokens):
    ...
    if token == "[CLS]" or token == "[SEP]" or token == "[MEM]":
      token_boundary[i] = 1
      continue
